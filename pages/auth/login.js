...
export default function Login() {
  ...
  function login(e) {
    {
      console.log(`${settings.APIURL}/Users?filters[$and][0][email][$eq]=`+e.target.email.value);
      e.preventDefault();
      axios
        ...
        .then(function (response) {
          ...
          if(date + 12 * 60 * 60 * 1000 < Date.now()){
            ...
            localStorage.setItem("pemail", e.target.email.value);
        ...
        .catch(function (error) {
          console.log(error);
          alert("An error occurred in the connection with the backend.");
        });
    }
  }
  function logout() {
    var pemail = localStorage.getItem("pemail");
    var id = localStorage.getItem("id");
    console.log("pemail=>", pemail, " id=>", id);
    if(pemail != null && id != null)
    {
      ...
      alert("There is no registered email.");
  }
